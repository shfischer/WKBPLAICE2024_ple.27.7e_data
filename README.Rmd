...
The SAM R package `stockassessment` is needed to run SAM and can be installed with
``` {r, eval = FALSE}
install.packages("stockassessment", repos = c("https://ices-tools-prod.r-universe.dev", "https://cloud.r-project.org"))
```

Finally, R package `cat3advice` for calculations of ICES category 3 methods is needed and can be installed with
``` {r, eval = FALSE}
